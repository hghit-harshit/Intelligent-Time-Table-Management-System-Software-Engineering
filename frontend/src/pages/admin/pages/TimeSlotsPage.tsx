...
import { PageHeader, Button, Modal, Card, DataTable } from "../../../shared";
import {
  fetchTimeSlotsFromApi,
  saveTimeSlotToApi,
  deleteTimeSlotFromApi,
...
export default function TimeSlotsPage() {
  ...
  const fetchSlots = async () => {
    try {
      setLoading(true);
      const data = await fetchTimeSlotsFromApi();
      setSlots(data);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await saveTimeSlotToApi(editingSlot?._id || null, payload);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteTimeSlotFromApi(deleteConfirm._id);
