...
const run = async () => {
    ...
    const slotByKey = new Map(slots.flatMap((slot) => (slot.occurrences || []).map((occurrence) => [
    ...
    for (const course of courseBlueprints) {
        const doc = await CourseModel.findOneAndUpdate({ code: course.code }, {
            $set: {
                code: course.code,
                name: course.name,
    ...
    const requirementDocs = Object.values(courseDocs).flatMap((courseDoc) => {
    ...
    if (requirementDocs.length) {
        ...
        await BatchCourseRequirementModel.bulkWrite(requirementDocs.map((doc) => ({
            updateOne: {
                filter: {
                    batchId: doc.batchId,
    ...
    for (const room of roomBlueprints) {
        await RoomModel.findOneAndUpdate({ name: room.name }, { $set: room }, { new: true, upsert: true });
    }
    const professorDocs = [];
    ...
    for (const code of Object.keys(courseDocs)) {
        ...
        await CourseModel.updateOne({ _id: courseDocs[code]._id }, {
            $set: {
                professorIds,
            },
        });
    ...
    console.log(`Batch Course Requirements: ${await BatchCourseRequirementModel.countDocuments()}`);
};
run()
    .then(async () => {
