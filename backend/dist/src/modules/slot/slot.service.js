    ...
    for (const incoming of occurrences) {
        for (const existingSlot of existingSlots) {
            for (const existingOccurrence of existingSlot.occurrences || []) {
                ...
                if (hasTimeOverlap(incoming.startTime, incoming.endTime, existingOccurrence.startTime, existingOccurrence.endTime)) {
                    return {
...
const validateSlotBusinessRules = async (payload, excludeId) => {
    ...
    if (conflict) {
        const err = new AppError("Occurrence conflicts with an existing slot", 409);
